# React Testing Library with Vitest Guide

## Quick Start

React Testing Library and Vitest are configured in this project. See the [official documentation](https://testing-library.com/docs/react-testing-library/intro/) for complete details.

Tests are co-located with the source they validate under `src/**/__tests__`. Root `tests/` is reserved for shared test support, global test types, and truly shared fixtures.
...
Import shared helpers with the test-only alias:

```tsx
import { renderWithProviders } from '@tests/support/test-utils';
import { createMockSession } from '@tests/support/mock-factories';
```

Do not import `@tests/*` from production source. Legacy source test-helper imports are forbidden.
