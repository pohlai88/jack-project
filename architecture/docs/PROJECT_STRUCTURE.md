...
The Afenda follows a Feature-Based Architecture adapted for Next.js App Router, combining the best practices of modular architecture with Next.js conventions. The main goal is to organize code around business capabilities while leveraging Next.js's powerful routing and server components.
...
Generated files and reports are intentionally kept outside source. Configurable outputs use `.artifacts/`, which is ignored by git. Framework/runtime defaults such as `.next/` and `node_modules/` stay on their standard paths for tooling compatibility.
...
tests/
├── support/                       # Shared test render helpers and mock factories
└── types/                         # Test-only global types

.artifacts/
├── cache/                         # Local tool caches when configurable
└── reports/                       # Generated local/CI reports
```
...
5. **Test Support Boundary**
   - Keep feature tests co-located under `src/**/__tests__`.
   - Use `@tests/support/*` only from test files and `tests/**`.
   - Production source under `src/**` must not import `@tests/*`.
