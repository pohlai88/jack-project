...
The Afenda uses **Drizzle ORM** with **PostgreSQL** and **pgvector** for AI embeddings.
...
- Connection string in `.env.local` (search_path should include `afenda,public,drizzle`):
...
Admin-editable DB content can be localized through `afenda.entity_translations`:
