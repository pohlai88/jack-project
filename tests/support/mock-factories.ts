 ...
 *
 * @example
 * import { createMockSession, createTypedAuthMock } from '@tests/support/mock-factories';
 *
 ...
 *
 * // Use in Mock
 * vi.mock('@/shared/lib/auth', () => ({ auth: authMock.mock }));
 */
export function createTypedAuthMock() {
  const mockFn = vi.fn<() => Promise<AuthResult>>();

  return {
    /** The underlying Vitest mock function */
    mock: mockFn,
...
export function createTypedTenantMock() {
  const mockFn = vi.fn<(tenantSlug: string) => Promise<MockTenant | undefined>>();
