...
/** Leg reaches label lane exactly — no hidden margin math. */
function getAnchorLegHeight(globalAnchorIndex: number) {
  return getAnchorLabelLaneOffset(globalAnchorIndex);
}
  ...
  milestones.forEach((milestone, milestoneIndex) => {
    milestone.anchors.forEach((anchor, anchorIndex) => {
      ...
      records.push({
        ...
        legHeight: getAnchorLegHeight(globalAnchorIndex),
      });
...
const ledgerTrackClass = cn('marketing-scanline-ledger__track', 'relative isolate min-h-[8.25rem] w-full');
...
const railBackdropClass = cn(
  ...
  'shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_0_20px_rgba(127,217,255,0.05)]',
);
...
const segmentProgressClass = cn(
  ...
  'h-[length:var(--landing-truth-ledger-rail-height)] opacity-[0.78]',
);
...
const hoverIntentBarBaseClass = cn(
  ...
  'transition-[left,width,opacity,transform] duration-[460ms] ease-[var(--marketing-ease)]',
  'motion-reduce:transition-none',
);

const hoverIntentBarVisibleClass = 'opacity-100 scale-x-100';
const hoverIntentBarHiddenClass = 'opacity-0 scale-x-50';
...
const anchorMarkerLegClass = cn(
  ...
  'opacity-[0.55]',
  'transition-[height,opacity,box-shadow] duration-[240ms] ease-out motion-reduce:transition-none',
  'group-[.is-complete]:opacity-[0.70]',
  'group-[.is-active]:opacity-[0.85]',
  'group-[.is-current]:opacity-100',
...
const anchorMarkerLabelClass = cn(
  ...
  'pointer-events-auto absolute left-1/2 top-[var(--truth-label-lane-offset)] z-[3]',
  'w-max min-w-[4.5rem] max-w-[7.75rem] -translate-x-1/2',
  'rounded-full border border-white/10 bg-black/[0.52]',
  'px-2.5 py-1 text-center uppercase',
  'font-[family-name:var(--marketing-mono)] text-[0.54rem] font-bold leading-[1.12]',
  'tracking-[0.105em] text-white/[0.62]',
  'shadow-[0_10px_24px_rgba(0,0,0,0.30),0_0_18px_var(--accent-glow)]',
  'backdrop-blur-md whitespace-normal break-words',
  '[display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2] [line-clamp:2]',
  'transition-[color,border-color,background-color,opacity,transform] duration-[240ms] ease-out',
  'motion-reduce:transition-none',
  'group-[.is-complete]:text-white/[0.72]',
  'group-[.is-active]:text-white/[0.86]',
  'group-[.is-current]:border-white/[0.24] group-[.is-current]:bg-black/[0.70] group-[.is-current]:text-white/[0.94]',
  'group-[.is-hovered]:border-white/[0.28] group-[.is-hovered]:bg-black/[0.78] group-[.is-hovered]:text-white',
  'group-[.is-hovered]:translate-y-[-0.08rem]',
);
...
const chapterLabelLayerClass = cn(
  ...
  'pointer-events-none absolute inset-x-0 top-[6.1rem] z-[2]',
...
function AnchorMarker({
  ...
  const { anchor, milestone, globalPosition, laneOffset, legHeight } = record;
  ...
  return (
    <a
      href={`#${anchor.id}`}
      className={cn(anchorMarkerClass, `is-${anchor.state}`, isCurrent && 'is-current', isHovered && 'is-hovered')}
...
function AnchorMarkerLayer({
  ...
  return (
    <div className={anchorMarkerLayerClass} aria-label="Truth ledger anchor labels" role="list">
      {anchors.map((record) => (
        <AnchorMarker
...
function HoverIntentBar({
  ...
  const barStyle = {
    ...
    '--truth-hover-bar-width': visible ? '6.5rem' : '0rem',
