...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  if (session?.user) {
    ...
    if (tenantSlugs.length === 1) {
      ...
      redirect(localizeHref(locale, '/select-tenant'));
    }
    // No memberships - stay on login page flow, will be handled below
  }

  return (
    <div className="min-h-screen flex items-center justify-center bg-linear-to-b from-background via-background to-muted/30 p-4 relative overflow-hidden">
      {/* Decorative elements */}
