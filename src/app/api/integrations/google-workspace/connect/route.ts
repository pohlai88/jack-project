...
export async function GET(request: Request) {
  ...
  const tenantCredentials = await getExternalOAuthTenantCredentials(tenantSlug, 'googleWorkspace');
  const credentials = getGoogleWorkspaceCredentials(tenantCredentials ?? undefined);
  if (!credentials) {
    return NextResponse.json({ error: 'Google Workspace OAuth is not configured' }, { status: 503 });
  }

  const redirectUri = getGoogleWorkspaceRedirectUri(request);
  const state = Buffer.from(JSON.stringify({ returnUrl: safeReturn, tenantSlug }), 'utf-8').toString('base64url');

  return NextResponse.redirect(buildGoogleWorkspaceAuthorizationUrl(redirectUri, state, credentials));
}
