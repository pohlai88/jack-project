...
import * as Base from '../../internals/sidebar/base';
import { createLinkItemRenderer } from '../../internals/sidebar/link-item';
import { createPageTreeRenderer } from '../../internals/sidebar/page-tree';
...
export function Sidebar({ footer, banner, collapsible = true, ...rest }: SidebarProps) {
  const {
    menuItems,
    slots,
              ...
              <SidebarCollapseTrigger
                className={cn(
                  buttonVariants({
                    variant: 'ghost',
                    size: 'icon',
                    className: 'mb-auto size-8 rounded-lg text-fd-muted-foreground',
                  }),
                )}
              >
                <SidebarIcon />
                ...
                <LinkItem
                  ...
                  className={cn(
                    buttonVariants({
                      variant: 'ghost',
                      size: 'icon',
                      className: 'size-8 rounded-lg p-2',
                    }),
                  )}
                  aria-label={item.label}
                >
      ...
      {({ collapsed, hovered, ref: asideRef, ...rest }) => (
        <>
          ...
          <div
            data-sidebar-panel=""
            className={cn(
              'fixed top-[calc(--spacing(4)+var(--fd-docs-row-3))] inset-s-4 z-10 flex rounded-xl border border-fd-border bg-fd-card/92 p-0.5 text-fd-muted-foreground shadow-lg backdrop-blur-md transition-opacity',
            ...
            <Base.SidebarCollapseTrigger
              className={cn(
                buttonVariants({
                  ...
                  size: 'icon',
                  className:
                    'size-9 rounded-lg text-fd-muted-foreground hover:bg-fd-accent/60 hover:text-fd-foreground',
                }),
              )}
            >
              <SidebarIcon />
