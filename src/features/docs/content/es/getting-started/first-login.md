...
Esta guía te lleva paso a paso por el primer inicio de sesión en Afenda, la selección de tu organización (tenant) y cómo aprovechar al máximo tu primera experiencia en el dashboard.

---

## Iniciar sesión con Auth0

Afenda usa **Auth0** para la autenticación. Podés iniciar sesión con:
...
1. Abrí la URL de Afenda que te dio tu organización (ej. `https://your-tenant.afenda.app` o el dominio personalizado de tu empresa).
...
5. Después de autenticarte, te redirigen de vuelta a Afenda.

> **Tip:** Si no ves la opción de inicio de sesión que esperás, tu tenant puede tener solo SSO o conexiones específicas habilitadas. Contactá a tu admin de Afenda o a IT por el método correcto.

---

## Selección de tenant (varias organizaciones)

Si pertenecés a **más de una organización** (tenant) en Afenda, después del login tenés que elegir en cuál trabajar.
...
- **Duración de la sesión** — Cuánto tiempo permanecés logueado depende de la configuración de Auth0 y Afenda de tu tenant. Puede pedírsete iniciar sesión de nuevo tras un tiempo de inactividad o tras un plazo fijo.
- **Cerrar sesión** — Usá el menú de cuenta (tu avatar o nombre en el encabezado) y elegí **Cerrar sesión**. Vas a tener que iniciar sesión de nuevo para acceder a Afenda.
- **Varios dispositivos** — Podés usar Afenda en más de un dispositivo. Cada sesión sigue las mismas reglas de seguridad y timeout. Cerrá sesión en dispositivos compartidos cuando termines.
- **Contraseña y MFA** — Los cambios de contraseña y la autenticación multifactor se gestionan en Auth0 (o tu proveedor SSO). Si necesitás restablecer la contraseña o actualizar MFA, usá el enlace de la página de login o contactá a IT o a tu admin de Afenda.
