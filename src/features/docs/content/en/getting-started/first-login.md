...
This guide walks you through signing in to Afenda for the first time, selecting your organization (tenant), and making the most of your first dashboard experience.

---

## Signing In with Auth0

Afenda uses **Auth0** for authentication. You can sign in with:
...
1. Open the Afenda URL provided by your organization (e.g. `https://your-tenant.afenda.app` or your company’s custom domain).
...
5. After authentication, you are redirected back to Afenda.

> **Tip:** If you don’t see the sign-in option you expect, your tenant may only have SSO or specific connections enabled. Contact your Afenda admin or IT for the correct login method.

---

## Tenant Selection (Multiple Organizations)

If you belong to **more than one organization** (tenant) in Afenda, you must choose which one to use after login.
...
- **Session duration** — How long you stay signed in depends on your tenant’s Auth0 and Afenda settings. You may be asked to sign in again after a period of inactivity or after a set time.
- **Signing out** — Use the account menu (your avatar or name in the header) and choose **Sign out** (or **Log out**). You will need to sign in again to access Afenda.
- **Multiple devices** — You can use Afenda on more than one device. Each session follows the same security and timeout rules. Sign out on shared devices when you are done.
- **Password and MFA** — Password changes and multi-factor authentication are managed in Auth0 (or your SSO provider). If you need to reset your password or update MFA, use the link from the login page or contact your IT or Afenda admin.
